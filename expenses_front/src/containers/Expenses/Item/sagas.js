...
import { api, bauth } from 'main/config';
...
export function* saveItem(action) {
  ...
  let data;
  if (expense.get('id') !== 0) {
    // yield call(alert, `${url}/${expense.get('id')}`);
    data = yield call(request, `${url}/${expense.get('id')}`, body, 'put', null, bauth);
  } else {
    //  yield call(alert, url);
    data = yield call(request, url, body, 'post', null, bauth);
  }
  // yield call(alert, data);
...
export function* deleteItem(action) {
  ...
  const data = yield call(request, `${url}/${expense.get('id')}`, null, 'del', null, bauth);
  if (data.err) {
    // yield call(alert, data.err.response.body.message);
